...
export default function ChatWidget({ mode = "student", orderContext, executeAction }: ChatWidgetProps) {
  ...
  const endpoint = isVendorMode ? "http://localhost:3001/api/vendor-chat" : "http://localhost:3001/api/chat";
  const offlineMessage = isVendorMode
    ? "Cannot reach the vendor copilot. Make sure the backend is running on port 3001."
    : "Cannot reach ByteBot. Make sure the backend is running on port 3001.";
