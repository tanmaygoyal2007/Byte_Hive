...
function getFetchErrorMessage(error: unknown, fallback: string) {
  if (error instanceof TypeError) {
      return `Cannot reach the payment backend${BACKEND_URL ? ` at ${BACKEND_URL}` : ""}.`;
  }
...
export async function createPaymentOrder(
  ...
  try {
    ...
    return res.json();
      ...
      throw new Error(getFetchErrorMessage(error, "Payment server is unavailable. Please try again."));
  }
}
